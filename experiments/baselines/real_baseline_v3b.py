"""
保存位置: Anla/experiments/baselines/real_baseline_v3b.py

实数对照实验 v3b — Anla 精确匹配版 homeostasis
=========================================================================

v3 的问题:
  v3 使用逐向量单位范数归一化 (每个 embedding 向量精确 ||w_i|| = 1.0)。
  这比 Anla 的 homeostasis 更严格:
    - 所有向量被强制投影到高维单位球面
    - MSE 梯度只能在球面切方向移动 embedding
    - 这天然倾向于在球面上产生有序排列
    - 100% NN 一致率可能是此几何约束的假象

v3b 修正:
  使用与 Anla ComplexEmbedding._apply_update() 完全一致的 batch-RMS 缩放:
    1. 计算所有 embedding 的 RMS: rms = sqrt(mean(||w_i||^2))
    2. 缩放因子 = 1.0 / rms
    3. 所有向量乘以缩放因子: w *= (1.0 / rms)
  ...
  如果 v3b 的 same_param NN 一致率仍然远高于 Anla → 确认 autograd vs manual_backward 差距
  如果 v3b 的 NN 下降到接近 Anla → 说明 v3 的结构是 homeostasis 策略差异造成的假象

用法:
  python -m Anla.real_baseline_v3b --configs A D
  python -m Anla.real_baseline_v3b
"""
...
# [Path Fix] 文件位置: Anla/experiments/baselines/real_baseline_v3b.py
...
# =====================================================================
#  实数模型 (v3b: batch-RMS homeostasis)
# =====================================================================
class RealBaselineInpainter(nn.Module):
    """
    v3b: 使用 Anla 精确匹配的 batch-RMS homeostasis。

    与 v3 的唯一区别:
      v3:  每个向量归一化到精确单位范数  ||w_i|| = 1.0
      v3b: 全局 RMS 缩放  w *= 1.0 / sqrt(mean(||w_i||^2))
           这允许不同向量有不同范数, 只保证全局 RMS ≈ 1.0

    对应 Anla 的 ComplexEmbedding._apply_update() line 108-120:
      batch_rms_energy = sqrt(moduli_sq.mean() + 1e-9)
    ...
    @torch.no_grad()
    def apply_embedding_homeostasis_batch_rms(self):
        """
        Anla 精确匹配的 batch-RMS homeostasis。

        直接翻译自 ComplexEmbedding._apply_update():
          moduli_sq = current_weights.abs().pow(2)
          batch_rms_energy = sqrt(moduli_sq.mean() + 1e-9)
          scaling_factor = 1.0 / (batch_rms_energy + 1e-9)
          current_weights.mul_(scaling_factor)

        对于实数:
          moduli_sq = w^2 (逐元素平方)
          batch_rms = sqrt(mean(w^2) + 1e-9)
          w *= 1.0 / batch_rms

        注意: Anla 对 ALL embedding 做 (包括 mask token), 我们也是。
        """
        w = self.embedding.weight.data  # (vocab+1, d_model)
        moduli_sq = w.pow(2)            # 实数的 |z|^2 = z^2
        batch_rms = torch.sqrt(moduli_sq.mean() + 1e-9)
        scaling_factor = 1.0 / (batch_rms + 1e-9)
...
def train_single_config(cfg, output_dir, device, log_interval=200,
    ...
    print(f"  FIX: Anla-exact batch-RMS homeostasis (NOT per-vector unit norm)")
    print("=" * 72)
    ...
    # 分离参数组: embedding 不受 weight_decay (与 v3 一致)
    ...
    for epoch in range(epochs):
        ...
        # ★ v3b 的核心区别: batch-RMS homeostasis (Anla 精确匹配)
        model.apply_embedding_homeostasis_batch_rms()
        ...
        if epoch % log_interval == 0:
            ...
            history["emb_norm_std"].append(emb_norm_std)
            ...
            print(f"  [{cfg['name']}] Epoch {epoch:05d} | "
                  ...
                  f"{test_str} | |emb|: {emb_norm_mean:.4f}±{emb_norm_std:.4f}")
    ...
    print(f"    (batch-RMS homeostasis: norms should vary, mean ≈ 1.0)")
...
def generate_report(results, output_dir):
    print()
    print("=" * 115)
    print("  Real Baseline v3b (Anla-exact batch-RMS homeostasis) — Report")
    print("=" * 115)
    ...
    for key in sorted(results.keys()):
        ...
        row = {
            ...
            "emb_norm_mean": res["embedding_norm_mean"],
            "emb_norm_std": res["embedding_norm_std"],
    ...
    # ---- 三方对比: Anla vs v3 vs v3b ----
    anla_path = os.path.join(_ANLA_ROOT, "Logs", "capacity_pressure_test", "COMPARISON_REPORT.json")
    v3_path = os.path.join(_ANLA_ROOT, "Logs", "real_baseline_v3", "REAL_BASELINE_V3_REPORT.json")

    anla_data = None
    v3_data = None

    if os.path.isfile(anla_path):
        with open(anla_path) as fp:
            anla_data = json.load(fp)
    if os.path.isfile(v3_path):
        with open(v3_path) as fp:
            v3_data = json.load(fp)

    if anla_data or v3_data:
        print()
        print("Three-Way Comparison: Anla (complex) vs Real v3 (per-vector norm) vs Real v3b (batch-RMS)")
        print("-" * 110)
        print(f"{'Config':<8} {'System':<25} {'TrainAcc':>9} {'NN%':>6} "
              f"{'|emb|μ':>8} {'|emb|σ':>8} {'Homeostasis':<20}")
        print("-" * 110)
        ...
        for bk in sorted(BASE_CONFIGS.keys()):
            ...
            # Anla
            if anla_data and bk in anla_data:
                ad = anla_data[bk]
                print(f"{bk:<8} {'Anla (complex)':<25} "
                      f"{ad.get('best_train_acc', -1):>8.1%} "
                      f"{ad.get('ring_nn_consistency', -1):>5.1%} "
                      f"{'~1.0':>8} {'~0.0':>8} "
                      f"{'batch-RMS (complex)':<20}")
                printed_any = True

            # v3 (per-vector norm)
            if v3_data:
                v3_sp = [r for r in v3_data if r["name"].startswith(f"{bk}_same_param")]
                if v3_sp:
                    r = v3_sp[0]
                    print(f"{'':8} {'Real v3 same_param':<25} "
                          f"{r['train_acc']:>8.1%} "
                          f"{r['nn_rate']:>5.1%} "
                          f"{r.get('emb_norm', 1.0):>8.4f} "
                          f"{'0.0000':>8} "
                          f"{'per-vector ||w||=1':<20}")
                    printed_any = True

            # v3b (batch-RMS)
            v3b_sp = [r for r in rows if r["name"].startswith(f"{bk}_same_param")]
            if v3b_sp:
                r = v3b_sp[0]
                print(f"{'':8} {'Real v3b same_param':<25} "
                      f"{r['train_acc']:>8.1%} "
                      f"{r['nn_rate']:>5.1%} "
                      f"{r['emb_norm_mean']:>8.4f} "
                      f"{r['emb_norm_std']:>8.4f} "
                      f"{'batch-RMS (Anla-match)':<20}")
                printed_any = True
        ...
        for bk in sorted(BASE_CONFIGS.keys()):
            ...
            if not v3b_sp:
                continue
            v3b_nn = v3b_sp[0]["nn_rate"]

            v3_nn = -1
            if v3_data:
                v3_sp = [r for r in v3_data if r["name"].startswith(f"{bk}_same_param")]
                if v3_sp:
                    v3_nn = v3_sp[0]["nn_rate"]

            anla_nn = -1
            if anla_data and bk in anla_data:
                anla_nn = anla_data[bk].get("ring_nn_consistency", -1)

            print(f"  [{bk}] NN%: Anla={anla_nn:.1%}, v3={v3_nn:.1%}, v3b={v3b_nn:.1%}")

            if v3_nn > 0 and v3b_nn > 0:
                if v3b_nn < v3_nn * 0.5:
                    print(f"       → v3b NN DROPPED significantly vs v3.")
                    print(f"         v3's 100% NN was likely a per-vector-norm artifact.")
                    if v3b_nn > anla_nn + 0.05:
                        print(f"         But v3b still > Anla → autograd advantage confirmed (weaker).")
                    else:
                        print(f"         v3b ≈ Anla → homeostasis strategy was the main driver, not gradient quality.")
                elif abs(v3b_nn - v3_nn) < 0.1:
                    print(f"       → v3b ≈ v3: homeostasis strategy makes no difference.")
                    if v3b_nn > anla_nn + 0.05:
                        print(f"         autograd >> manual_backward confirmed for structure learning.")
                    else:
                        print(f"         Both comparable to Anla → architecture is the bottleneck.")
                else:
                    print(f"       → v3b < v3 but still significant: partial artifact, partial real.")
            print()

    # 保存
    report_path = os.path.join(output_dir, "REAL_BASELINE_V3B_REPORT.json")
...
def main():
    parser = argparse.ArgumentParser(
        description="Real baseline v3b: Anla-exact batch-RMS homeostasis")
    ...
    parser.add_argument("--output-dir", type=str,
                        default=os.path.join(_ANLA_ROOT, "Logs", "real_baseline_v3b"))
