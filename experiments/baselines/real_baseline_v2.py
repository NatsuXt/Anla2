"""
保存位置: Anla/experiments/baselines/real_baseline_v2.py

修正版实数对照实验
=========================================================================

v1 的 bug:
    embedding 输出乘了 sqrt(d_model), 但目标是未缩放的原始 embedding 权重。
    这导致输出尺度 (~8) 与目标尺度 (~1) 严重不匹配,
    准确率被压低到 ~7%, 无法公平对比。

v2 修正:
    1. 移除 sqrt(d_model) 缩放 (与 Anla 的 ComplexEmbedding 对齐)
    2. 使用与 Anla 等价的 MSE loss (无缩放, 在 masked 位置直接计算)
    3. 保持其他一切不变: 同样的 nn.TransformerEncoderLayer, autograd, Adam

用法:
    python -m Anla.real_baseline_v2 --configs A D
    python -m Anla.real_baseline_v2
"""
...
# [Path Fix] 文件位置: Anla/experiments/baselines/real_baseline_v2.py
...
class RealBaselineInpainter(nn.Module):
    """
    修正版: 不做 sqrt(d_model) 缩放, 与 Anla 的 ComplexEmbedding 行为对齐。

    架构:
        nn.Embedding (初始化为单位范数)
        → SinusoidalPositionalEncoding (加法, 不改变量级)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        ★ 关键修正: 不乘 sqrt(d_model), 保持 embedding 原始尺度。
           Anla 的 ComplexEmbedding.forward 也不做缩放。
        """
        z = self.embedding(x)          # (B, S, d_model), norm ≈ 1
        z = self.pos_encoder(z)         # 加法, norm 变化不大
        z = self.transformer_layer(z)   # Pre-LN + 残差, 输出尺度 ≈ 输入尺度
        return z
...
def evaluate_nearest_neighbor(z_pred, target_ids, all_embeds, chunk_size=256):
    ...
    if vocab_size <= 2048:
        ...
        for i in range(0, n_masked, chunk_size):
            ...
            d = (z_masked[i:end].unsqueeze(1) - all_embeds.unsqueeze(0)).pow(2).sum(-1)
...
def quick_tda_audit(w_np, k=6):
    ...
    mask_f = np.isfinite(dgm1[:, 0]) & np.isfinite(dgm1[:, 1])
    dgm1 = dgm1[mask_f]
    if dgm1.size == 0:
        return {"available": True, "h1_count": 0,
                "dominant_persistence": 0.0, "dominance_ratio": 0.0}
...
def train_single_config(cfg, output_dir, device, log_interval=200,
    ...
    print()
    print("=" * 72)
    print(f"  [REAL v2] {cfg['name']}")
    print(f"  vocab={vocab_size}, d_model={d_model} (real, NO sqrt scaling), "
    ...
    for epoch in range(epochs):
        ...
        # 目标: embedding 原型 (无缩放, 与模型输出在同一尺度)
    ...
    # ---- Embedding + TDA ----
    w_np = all_embeds.detach().cpu().numpy().astype(np.float64)
    feat_path = os.path.join(output_dir, "embedding.npz")
    np.savez(feat_path, x_real=w_np)
    tda = quick_tda_audit(w_np, k=6)
    ...
    print(f"\n  [{cfg['name']}] Done:")
    print(f"    Train: {best_train_acc:.2%} (epoch {last_best_epoch})")
    print(f"    Test:  {final_test_acc:.2%} ± {final_test_std:.2%}")
    print(f"    NN:    {nn_rate:.1%}")
...
def generate_report(results, output_dir):
    print()
    print("=" * 105)
    print("  Real Baseline v2 (corrected) — Report")
    print("=" * 105)
    ...
    if os.path.isfile(anla_path):
        with open(anla_path, "r") as f:
            anla_data = json.load(f)

        print()
        print("Direct Comparison: Anla (Complex, manual_backward) vs Real (autograd)")
        print("-" * 95)
        print(f"{'Config':<8} {'Anla Train':>11} {'R(SD) Train':>12} {'R(SP) Train':>12} "
              f"{'Anla NN%':>9} {'R(SD) NN%':>10} {'R(SP) NN%':>10} "
              f"{'Anla H1dom':>11} {'R(SP) H1dom':>12}")
        print("-" * 95)
        ...
        for bk in sorted(BASE_CONFIGS.keys()):
            ...
            ah = ad.get("tda_dominant", -1)

            sd = [r for r in rows if r["name"].startswith(f"{bk}_same_dim")]
            sp = [r for r in rows if r["name"].startswith(f"{bk}_same_param")]

            def g(v): return f"{v:.1%}" if v >= 0 else "---"
            def h(v): return f"{v:.4f}" if v >= 0 else "---"

            sdt = sd[0]["train_acc"] if sd else -1
            spt = sp[0]["train_acc"] if sp else -1
            sdn = sd[0]["nn_rate"] if sd else -1
            spn = sp[0]["nn_rate"] if sp else -1
            sph = sp[0]["h1_dominant"] if sp else -1

            print(f"{bk:<8} {g(at):>11} {g(sdt):>12} {g(spt):>12} "
                  f"{g(an):>9} {g(sdn):>10} {g(spn):>10} "
                  f"{h(ah):>11} {h(sph):>12}")
        ...
        for bk in sorted(BASE_CONFIGS.keys()):
            ...
            if nn_diff > 0.05:
                ...
                nn_verdict = f"Similar ({spn:.1%} vs {an:.1%})"
            ...
            if at > spt + 0.1 and spn > an + 0.05:
                ...
                print(f"         This indicates manual_backward transmits "
                      f"enough signal for task-solving")
                print(f"         but NOT enough structural gradient to shape "
                      f"the embedding geometry.")
            print()

    # 保存
    report_path = os.path.join(output_dir, "REAL_BASELINE_V2_REPORT.json")
...
def main():
    parser = argparse.ArgumentParser(
        description="Real baseline v2 (corrected): no sqrt(d_model) scaling")
    ...
    parser.add_argument("--output-dir", type=str,
                        default=os.path.join(_ANLA_ROOT, "Logs", "real_baseline_v2"))
