"""
保存位置: Anla/experiments/capacity/capacity_pressure_test_v6.py

容量压力测试 v6 — 全纯非线性 Encoder-HoloTransformer-Decoder 架构
=========================================================================
...
v5 → v6 变更:
    ...
    [1] FFN 激活函数: PhaseTwist → HolomorphicActivation
        ...
        PhaseTwist 的问题:
            · 用固定函数形式 (γ·r, β·cos(θ-φ)) 硬编码 AM↔PM 耦合
            · γ, β 是全局标量, 所有 token/维度共享同一耦合强度
            · 固定耦合在每次前向传播中无条件施加, 3 层叠加后
              形成耦合振荡器反馈环路, 导致后期训练的系统性振荡
            · φ 参数打破旋转对称性

        HolomorphicActivation: f(z) = z + α·z²
            · 全纯函数, Cauchy-Riemann 方程保证 AM↔PM 耦合自然涌现
            · AM→PM: Δθ ≈ |α|·r·sin(θ+∠α) (同时依赖模长和相位)
            · PM→AM: ΔR/R ≈ |α|·r·cos(θ+∠α) (调和共轭对, 非人为设计)
            · 逐通道复数参数 α, per-dim 差异化耦合 (vs 全局标量)
            · 反向传播简洁: f 全纯 ⟹ df/dz*=0, 只需 1 个偏导项
            · 频率创造: z² 产生 2θ 二次谐波, 弥补频率覆盖缺口
            · RMSNorm 保证 |z|~O(1), 绕过 Liouville 定理的全局有界限制

        跨维度 AM↔PM 耦合:
            不在激活函数层面实现, 而通过 FFN 三段式结构涌现:
                W1: D → D_ffn (跨维度线性混合)
                Act: 逐元素全纯非线性 (在混合信号上施加 AM↔PM)
                W2: D_ffn → D (投影回原始维度)
            跨维度耦合的路由由 W1, W2 的可学习权重控制。

        (由 activation.py v6 + transformer_block.py v6 实现,
         本脚本的训练逻辑无需修改)

    以下 v5 特性保持不变:
    [v5.1] Encoder-HoloTransformer-Decoder 架构
    [v5.2] BERT 风格多位置 Masking
    [v5.4] Path A 径向投影 (project_out_radial)
    [v5.5] 线性 Warmup 学习率调度
    [v5.6] Block 数量: 3

用法:
    python -m Anla.experiments.capacity.capacity_pressure_test_v6
    python -m Anla.experiments.capacity.capacity_pressure_test_v6 --configs B
    python -m Anla.experiments.capacity.capacity_pressure_test_v6 --configs B --mask-mode span
"""
...
# =====================================================================
#  [v6] 模型 — Encoder-HoloTransformer-Decoder (全纯非线性版)
# =====================================================================
class AnlaManifoldInpainter_v6(nn.Module):
    """
    v6 Encoder-HoloTransformer-Decoder 架构。

    v5 → v6 的唯一架构变更:
        TransformerBlock 内部的 FFN 激活函数从 PhaseTwist 替换为
        HolomorphicActivation (f(z) = z + α·z²)。
        此变更完全封装在 ComplexTransformerBlock 内部,
        模型类的代码结构与 v5 完全一致。
    ...
    信号路径:
        input_ids
         ...
         → TransformerBlock × num_blocks — 潜在空间中的多层推理
             ...
             HolomorphicActivation 提供:
                 · 全纯非线性 (打破线性可折叠性)
                 · AM↔PM 耦合 (由 Cauchy-Riemann 方程保证)
                 · 二次谐波频率创造 (z² → 2θ)
         → Decoder:
    ...
    Encoder 的设计 (全纯映射, 保角变换):
        ...
        非线性由后续 Block 内的 HolomorphicActivation 提供。
    ...
    def __init__(self, vocab_size: int, d_model: int, num_heads: int,
        ...
        # [v6] Block 内部的 FFN 使用 HolomorphicActivation (f(z)=z+α·z²)
        #      替代 v5 的 PhaseTwist。此变更封装在 ComplexTransformerBlock 中。
...
def train_single_config(
    ...
    # [v6] 更新打印信息: PhaseTwist → HolomorphicActivation
    print(f"  [v6] Encoder-Decoder 架构, HolomorphicActivation (z+α·z²), "
    ...
    # ---- 模型 ----
    # [v6] 使用 AnlaManifoldInpainter_v6 (内部 TransformerBlock 已切换激活函数)
...
def main():
    parser = argparse.ArgumentParser(
        description="Anla 容量压力测试 v6: 全纯非线性 Encoder-Decoder 架构"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-dir", type=str,
        default=os.path.join(_ANLA_ROOT, "Logs", "capacity_pressure_test_v6"),
        help="输出根目录"
    )
    parser.add_argument(
